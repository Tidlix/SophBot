        ...
        public async Task HandleEventAsync(DiscordClient s, ComponentInteractionCreatedEventArgs e)
        {
            switch (e.Id)
            {
                case "ruleAcceptButton":
                    ...
                    break;
                default:
                    if (e.Id.Contains("rrButton_"))
                    {
                        await reactionRole(e);
                    }
                    if (e.Id.Contains("deleteUserWarningButton_"))
                    {
                        await deleteWarning(e);
                    }
                    break;
            }
        }
...
#pragma warning disable CS8602
            ...
            try
            {
                DiscordRole memberRole = await guild.getRoleAsync(TDiscordGuild.Role.Member);
        ...
        private async Task deleteWarning(ComponentInteractionCreatedEventArgs e)
        {
            await e.Interaction.DeferAsync(true);
            
            string warnIdString = e.Id.Substring(e.Id.IndexOf('_') + 1);

            ulong.TryParse(warnIdString, out ulong warnId);

            await TDatabase.Warnings.deleteAsnyc(warnId);
            await e.Interaction.EditOriginalResponseAsync(new DiscordWebhookBuilder().WithContent("Verwarnung aufgehoben!"));
        }
    }
}