...
namespace SophBot.Twitch.Events
{
    public static class StreamOnlineEvents
    {
        public static async Task OnStreamOnlineAsync(object? sender, OnStreamOnlineArgs args)
        {
            ...
            {
                DiscordChannel channel = await DiscordEngine.Client.GetChannelAsync((ulong)current["discord-channel"]);
                DiscordRole role = await channel.Guild.GetRoleAsync((ulong)current["mention-role"]);
                DiscordComponent[] components = [
                    new DiscordTextDisplayComponent($"# {args.Stream.UserName} ist nun Live!"),
                        new DiscordSeparatorComponent(true),
                        new DiscordTextDisplayComponent($"## [{args.Stream.Title}](https://twitch.tv/{args.Channel.ToLower()})"),
                        new DiscordMediaGalleryComponent(new DiscordMediaGalleryItem(args.Stream.ThumbnailUrl, "Stream Thumbnail", false)),
                        new DiscordSeparatorComponent(true),
                ...
                await channel.SendMessageAsync(new DiscordMessageBuilder().EnableV2Components().AddContainerComponent(new(components, false, DiscordColor.Purple)));
            }
